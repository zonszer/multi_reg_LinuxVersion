...
import numpy
from utilss.utilss import pairwise_distance
...
class ShapeContext(object):
    ...
    def _dist2(self, x, c):
        ...
        for i in range(N):
            for j in range(len(c)):
                ...
                result[i, j]  = numpy.linalg.norm(x[i], c[j])        #change 1
        return result
    ...
    def compute(self, X, r=None):
        ...
        r_bin_edges = logspace(math.log10(self.r_inner), math.log10(self.r_outer), self.nbins_r)                #add  2: math.
        ...
        theta_array_q = 1 + numpy.floor(theta_array_2 / (2 * math.pi / self.nbins_theta))            #chang to int() and math.     change2 math.int  change3 :numpy.
        theta_array_q = array(theta_array_q, dtype=int)
